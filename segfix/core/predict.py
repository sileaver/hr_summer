...
def predict(model,
    ...
    gray_saved_dir =  os.path.join(save_dir, 'gray_prediction')
    gray_whole_saved_dir =  os.path.join(save_dir, 'gray_whole_prediction')
    ...
    with paddle.no_grad():
        for i, im_path in enumerate(img_lists[local_rank]):
            ...
            # save original perdiction
            pred_im = pred
            pred_saved_path = os.path.join(gray_saved_dir, im_file)
            mkdir(pred_saved_path)
            cv2.imwrite(pred_saved_path, pred_im)

            # put all prediction into a dir
            pred_im = pred
            im_file = im_file.split("/")[-1]
            pred_saved_path = os.path.join(gray_whole_saved_dir, im_file)
            mkdir(pred_saved_path)
            cv2.imwrite(pred_saved_path, pred_im)
