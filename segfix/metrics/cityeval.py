...
def getPrediction( args, groundTruthFile ):
    ...
    if not args.predictionPath:
        ...
        if 'CITYSCAPES_RESULTS' in os.environ:
            ...
            #--------------prediction file (put all preditions in the same file, program will pair them automatically----------------
            #rootPath = os.path.join("/media/yly/AIvehicle/save_color/",'results')
            # rootPath = os.path.join("/media/yly/AIvehicle/prediction/","gtFine" , "val") 
            
            # for segfix
            # rootPath = os.path.join("/home/aistudio/contrast_seg_paddle/output","label_w_segfix") 

            rootPath = os.path.join("/home/aistudio/contrast_seg_paddle/output/result","gray_whole_prediction") 
    ...
    filePattern = "{}_{}_{}_leftImg8bit.png".format( csFile.city , csFile.sequenceNb , csFile.frameNb )
    predictionFile = None
...
#----------------------------groundTruth file---------------------------------
args.groundTruthSearch  = os.path.join( "/home/aistudio/data/data64550/cityscapes/" , "gtFine" , "val" , "*", "*_gtFine_labelTrainIds.png" )
# args.groundTruthSearch  = os.path.join( "/home/aistudio/output","*_gtFine_labelTrainIds.png" )
#args.groundTruthSearch  = os.path.join( "/media/yly/AIvehicle/CITYSCAPES_DATASET/" , "gtFine" , "val" , "frankfurt", "*_gtFine_labelTrainIds.png" )
...
def evaluatePair(predictionImgFileName, groundTruthImgFileName, confMatrix, instanceStats, perImageStats, args):
    ...
    try:
        ...
        #--------------convert the 3D channel to 1D channel--------------------------    
        predictionImg = predictionImg.convert('L')
    ...
    try:
        ...
        #-------------resize the groudTruthImg to match the predict one if necessary, note to change the size according to your predictions----------------
        #groundTruthImg=groundTruthImg.resize((128,64),Image.NEAREST) #(for encoder)
        #groundTruthImg=groundTruthImg.resize((1024,512),Image.NEAREST) #(for resized decoder)
