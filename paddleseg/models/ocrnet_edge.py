...
class SegHead(nn.Layer):
    def __init__(self, in_chan, mid_chan, n_classes=1):
...
class FeatureFusionModule(nn.Layer):
    def __init__(self, out_chan):
        ...
        self.conv2 = nn.Conv2D(
            ...
            padding=0,
            bias_attr=None)
        # self.relu = layers.FReLU(out_chan)
        self.relu = nn.ReLU()
...
class OCRNet(nn.Layer):
    ...
    def forward(self, x):
        ...
        logit_list,edge_feature  = self.head(feats)
        logit_list.append(edge_feature)
...
class OCRHead(nn.Layer):
    ...
    def __init__(self,
        ...
        # self.conv3x3_ocr = layers.RepVGGBlockBNRelu(
        ...
        self.detail_head = SegHead(512,256,1)
        self.ffm = FeatureFusionModule(512)
        self.down = layers.ConvBNReLU(513,512,1)
        self.init_weight()
    ...
    def forward(self, feat_list):
        ...
        edge_feature = self.detail_head(pixels)
        pixels_edge = paddle.concat([pixels, edge_feature], axis=1)
        pixels_edge = self.down(pixels_edge)
        pixels_edge = self.ffm(pixels_edge)
        pixels = pixels_edge
        object_regions = self.spatial_gather(pixels, soft_regions)
        ocr = self.spatial_ocr(pixels, object_regions)

        logit = self.cls_head(ocr)
        return [logit, soft_regions],edge_feature
